# 다음 세션 인수인계 (2026-08-20 갱신 · 2차)

> **읽는 순서**: 이 파일 → `HANDOFF.md`(전체 경위·검정 결과) → 필요한 소스 docstring
> `HANDOFF.md`가 기준 문서다. 이 파일은 **지금 어디까지 왔고 다음에 뭘 하면 되는지**만 적는다.

---

## 지금 상태 — 완료된 것

### 자료 (완결)
```
data/raw/lettuce_daily_raw.csv     2,164,359행 / 268MB
                                   전국 32개 공영도매시장 전수
                                   2018-01-02 ~ 2026-08-11, 전 거래일
                                   전북산 476,188행 / 시군 14개 / 읍면 63개
data/raw/daily_weather_lettuce.csv 전북 ASOS 9지점 x 11필드
data/raw/daily_weather_aws.csv     전북 AWS 8지점
data/raw/daily_weather_competitors.csv  경쟁 7개 시도 26지점
```
검수(`audit_lettuce_daily.py`) 전 항목 통과. 목록 감사(`audit_hardcoded_lists.py`)도 통과.

### 모델 (확정)
| 용도 | 구성 | 성적 |
|---|---|---|
| 월간 h=1개월 | 계절+가격시차, Ridge+스미어링 | CV 23.03% / 홀드아웃 11.17% (계절평균 25.45%/12.67%) |
| 주간 h=1주 | +광, Ridge+GBM 앙상블 | CV 21.50%, 6/6 전승 (주차평균 31.17%) |
| 2027 전망 | 1~3개월 모델 / 4개월+ 계절평균×수준보정 | 지평별 22~23% |
| 품종별(포기찹·청상추·적상추) | 같은 주간 모델을 품종마다 다시 | 계열 104주 미만이면 **예측 안 냄** |
| 급등 상태 h=1주 | 로지스틱 | AUC 0.950 (지속성 0.857) 개선 |
| 급등 **시작 시기** | 로지스틱 | **판정불가** — 달력 이상 없음 |
| 출하처 선택 | 같은 주·같은 품종 대응짝 | 관행 대비 +10.0% [+8.0, +12.0] |

### 웹앱 (2026-08-14 기준 · 실서버 확인 완료)

공개 주소 <https://erato0310.github.io/crop-price-forecast/>

```
webapp/lettuce.html                     한 파일에 화면·스타일·렌더링 전부
webapp/data/lettuce_app.json     1.2MB  전체(주력 17품종 합산) — 첫 화면
webapp/data/lettuce_destinations.json  427KB  출하처 도매시장·품종 (기간별)
webapp/data/lettuce_forecast_2027.json 194KB
webapp/data/jeonbuk_eup_geo.json 169KB  읍면 경계 — 드릴다운할 때만
webapp/data/lettuce_surge.json    18KB  여름·가을 급등기
webapp/data/lettuce_var_chung.json     977KB  청상추 ─┐
webapp/data/lettuce_var_jeok.json      851KB  적상추  ├ 품종을 고를 때만
webapp/data/lettuce_var_pogichap.json  560KB  포기찹 ─┘
```
첫 화면이 받는 것은 1.9MB. 나머지는 필요할 때만 받는다.

띄우는 법:
```bash
cd "C:/Users/지리산 살래농장/Documents/project 1/crop-price-forecast" && .venv/Scripts/python.exe -m http.server 8731 --directory webapp
```
→ `http://localhost:8731/lettuce.html`

> **캐시 주의.** `python http.server`는 캐시 방지 헤더를 안 보낸다. 고친 게 안 보이면
> `?v=2` 같은 걸 붙이거나 Ctrl+Shift+R. 이것 때문에 "수정이 하나도 안 됐다"고
> 한참 헤맸다. 공개 사이트도 마찬가지다.

**화면 구성**
- 3단계 확대: 전북 → 시군(3.1배) → 읍면(9.4배). 뒤로가기도 한 칸씩 나온다.
- 확대해 들어가면 지도 옆에 그 지역 요약(값·물량·출하처·품종·급등기·예측)이 뜬다.
- **설명은 카드마다 오른쪽 위 물음표 안에** 있다. 카드 본문에는 한 줄만 둔다.
  한계에 관한 말은 접어 두되 지우지 않았다 — 물음표 안 경고 상자에 있다.
- 상단 걸개 둘: **기간**(8주/6개월/1년/3년/전체/직접) · **품종**(전체/포기찹/청상추/적상추)
- 급등기가 진행 중이면 맨 위에 주의 배너. 판단 기준은 브라우저 날짜가 아니라
  **자료의 마지막 주**다.

**얇은 계열을 다루는 규칙(세 군데에 같은 잣대)**
값만 보고 세우면 몇 주치뿐인 곳이 1위로 올라온다. 실제로 세 번 겪었다.
1. 읍면 표 — 남원 산동면 0.2t·1주가 9,622원으로 1위
2. 시군 표(품종 선택 시) — 군산 청상추 22주가 7,714원으로 1위
3. 지도 색 척도 — 위 둘이 척도를 다 차지해 진짜 산지가 옅어 보임

→ 순위·지표·색척도 **셋 다에서 빼고** 표 아래로 내린다. 표에는 남긴다(실제 거래다).

---

## 2026-08-13~14에 새로 만든 것

| 무엇 | 스크립트 | 결과 |
|---|---|---|
| 읍면 경계 | `build_geo.py` (확장) | 시군과 **같은 투영**. 이름은 지도 원본 그대로 두고 가격자료 라벨에 맞춰 고치지 않았다. 물량 기준 48.4%만 폴리곤이 붙는다 |
| 여름·가을 급등기 | `analyze_autumn_surge.py` | 9년 중 9년 발생. 시작 중앙값 07-27, 지속 7주, 꼭대기 상반기 대비 4.9배 |
| 출하처 도매시장 | `export_destinations.py` | 시군·읍면별 상위 6곳 + 품종 구성. 익산=서울가락 67%, 운봉읍=부산엄궁 83% |
| 출하처 **선택 검정** | `analyze_market_choice.py` | 같은 주·같은 품종 대응짝 13,909개. 관행 대비 **+10.0%** |
| 품종 3분할 | `export_by_variety.py` | 포기찹·청상추·적상추. 상위 3품종이 물량의 90.8% |

### 품종 분할에서 지킨 원칙
셋으로 나누면 계열이 3분의 1로 얇아진다. **값과 예측을 다르게 다룬다.**
- 값(평균·최고·최저·물량) — 세는 것이므로 자료가 있으면 낸다
- 예측·검증 성적 — **104주 미만이면 아예 내지 않는다.** 짧은 자료로 낸 예측은
  맞는지 확인할 방법이 없다. 화면에는 `자료 얇음`으로 뜬다

실측: 청상추는 군산 빼고 전 시군 가능 / 포기찹은 6곳 미달(고창 46주·임실 44주·부안 37주)
/ 군산은 세 품종 다 부족.

---

## 2026-08-20에 한 것 — 지역 가격차와 유통구조

### 일교차 가설 — 기각 (HANDOFF 1-E)
사용자 제안(일교차가 크면 잎이 두꺼워져 상품성이 오른다)을 **다섯 설계**로 검정했다.
시군 간 상관 / 계절 격차 / 시군 내 주별 값 / 시군 내 주별 등급 / 문턱 13개(6~12℃).
**전부 판정불가.** 시군 내 4,321주 표본에서 r≈0이라 표본 부족이 아니라 신호가 없다.
이 자료로 잡을 수 있는 최소 효과가 ±1.3~2.1%인데 실측은 그보다 작다.

> 여기서 얻은 교훈 두 개는 다음에도 쓰인다.
> - **집단 비교의 함정.** 10℃로 가르면 p=0.039가 나오는데 무주 한 점을 빼면 사라진다.
>   집단 간 유의성이 집단 내에서 사라지거나 뒤집히면 그건 지역의 다른 성질을 잡은 것이다.
> - **왜 안 잡히나.** 특 등급이 81.3%다. 등급이 이렇게 몰려 있으면 미세한 품질차가
>   실릴 자리가 없다.

### 지역 가격차의 정체 — 포장 규격 (HANDOFF 1-F)
**얇은 시군이 폭을 두 배로 부풀리고 있었다.** 군산 4t·무주 10t·정읍 30t·임실 40t·
고창 56t 다섯 곳이 전북 물량의 1%인데 폭의 절반을 만든다. 빼면 0.474 -> 0.226.

남은 9곳에서 걸린 것은 **소형(≤2.5kg) 상자 비율** 하나다(r=+0.765).
지역과 무관하게 확인했다 — 같은 주·시장·품종·**등급** 안에서 2kg이 4kg보다
kg당 **23% 더** 받는다(셀 1,462개). 소형상자 비율만으로 지역차 폭의 **83%**,
분산의 57%를 재현한다.

> 셀 통제로는 8%밖에 안 줄었던 이유도 알았다. 남원은 88%가 2kg이고 부안은 0.1%라
> 같은 규격끼리 견줄 셀이 드물다. **차이가 셀 안이 아니라 구성에 있었다.**

### 유통 단계별 가격 (문서 별도)
KAMIS 소매·도매를 품종별로 받아 우리 경락가와 붙였다(`fetch_retail_kamis.py`).
최근 1년 52주.

```
적상추  산지 3,996 / 중도매 6,799 / 소매 12,695   농가몫 30.0%
청상추  산지 4,716 / 중도매 6,856 / 소매 13,729   농가몫 33.3%
산지 비쌀 때 농가몫 48% (2.0배) / 쌀 때 18% (5~6배)
```
주간 변화 상관이 +0.24/+0.09로 매우 낮다(수준 상관은 0.78) — **소매가가 산지가의
주간 등락을 거의 안 따라간다.** 이게 이 분석의 기여다. 연평균 통계로는 안 보인다.

문서: `docs/유통단계별_가격_2026-08-20.html` (9절, 국제 비교 포함)

---

## 2026-08-20 후반에 한 것

### aT 상추 출하비용률 — 확보. 앞선 추정의 방향이 반대였다
막혀 있던 건이다. KAMIS 「품목별 유통비용」은 `product.do?action=list&
itemclasscode=NN&itemcode=NN` GET 한 방으로 받힌다. 스크립트로 만들었다
(`fetch_at_margin.py`, 46품목 x 1998~2024, 1,222행).

**상추 출하단계 비용률 5.9%** (소비자가 100 기준). 문서가 대입했던 전 품목 평균
9.4%보다 **작다.** 그래서 농가수취율은 20%가 아니라 **25~27%** — 미국 신선채소
(23~28%)와 같은 범위다.

> 문서에는 "상추는 부피 크고 무르니 출하비용 비중이 평균보다 **클** 것"이라 적었는데
> 틀렸다. 이 %는 전부 **소비자가격이 분모**다. 상추는 소매가가 kg당 12,000원대라
> 같은 운송비도 비중이 작게 잡힌다. **부피·부패성은 절대 비용을 키우지만 비중을
> 키우지는 않는다.** 1-E의 '집단 비교의 함정'과 같은 종류의 실수다.

주의: **상추는 2013년부터 조사 품목에서 빠졌다.** 2012년 행은 2011년과 9개 값이
전부 같아 이월로 보인다(같은 현상 17품목 61건). 마지막 실조사는 **2011년**이다.
`fetch_at_margin.py --check-drop`이 이걸 센다. 문서 6절·HANDOFF 1-G에 반영했다.

### 10작물 라인 — 저장소에서 제거
방침이 상추 단일 작목인데 「다음 할 일」에 10작물 버그 수정이 올라와 있어 한동안
그쪽을 파다가 되돌렸다. 버그를 고치는 대신 라인을 걷어냈다. 추적 파일 **102개**
(src 29 · data 3 · outputs 70). 자세한 목록은 HANDOFF 2절 「제거된 것」.

- 남긴 것: `data/raw/jeonbuk_origin_allcrops_raw.csv` — `audit_lettuce_daily.py`가
  상추 전수 자료 교차검증에 쓴다.
- 검증: 남은 43개 스크립트 구문·import 정상, 검수 2종 결과 변화 없음.

### 남원 '산간 전환' — 라벨 이동이었다 (HANDOFF 1-H)
10.5에 "산간 비중 5% -> 28%, 구조 변화 중"이라 적어 뒀던 것이 틀렸다.
부산엄궁 한 시장만 떼면 그대로 보인다.

```
연도    2018 2019 2020 2021 2022 2023  2024 2025 2026
금지면   419  546  511  544  559  585   128    0    0
운봉읍     0    0    0    0    0    0   394  876  997
```

같은 출하자라는 증거 넷: ①2024-05 한 달 공백 말고는 **끊김 없음** ②지문 일치
(**2kg 상자 100%**·경매 100%·상등급 98→100%) ③라벨에 **리(里)가 없다**
(2,624t 중 동천리 3.3t) ④**연중 출하**(1월 26t·7월 114t·12월 76t) — 해발 500m
노지 고랭지면 여름에 몰려야 한다.

보정하면 **5.4%(2018) -> 9.8%(2025)**. 겉보기 증가분의 4분의 3이 라벨 하나다.
(늘긴 늘었고 그건 진짜다 — 아영면 135->288t, 광주서부행.)

> **밭이 실제로 옮겨갔을 가능성은 배제 못 한다.** 말할 수 있는 건 출하 경로가
> 이어졌고 포장·등급·거래가 그대로라는 것까지다. 그래서 새 산지로 세지 않는다.
>
> **일반화할 것: 읍면 시계열에서 계단처럼 뛰는 값을 보면, 산지 이동으로 읽기 전에
> 시장별로 쪼개 끊김을 확인한다.** 다른 시군에서도 같은 일이 일어날 수 있다.

스크립트 `analyze_namwon_shift.py`. 산간 읍면 목록은 사람 판단이라
`audit_hardcoded_lists.py` 5-d에 등록했다.

---

## 일교차를 세 가지로 다시 검정했다 — 전부 판정불가 (HANDOFF 1-E 설계 6~8)

기각된 일교차 가설에 사용자가 세 갈래로 다시 물었고, 셋 다 해봤다.

| | 물음 | 결과 |
|---|---|---|
| 설계 6 | 물량으로 **가중**하면? | 시군 간은 가중하면 유효표본이 3.67로 무너진다. 시군 내는 표본이 버티는데 결과가 안 변한다 |
| 설계 7 | 일교차 말고 **규모와 같이** 넣으면? | 규모는 방향이 맞다(+). 그런데 n=14라 계수가 안 잡힌다 |
| 설계 8 | 1-F처럼 **대응짝**으로 옮기면? | 안 된다. 그리고 이유가 구조적이다 |

### 여기서 나온 것이 이 프로젝트의 경계다 — 다음에도 계속 쓰인다

새 가설이 나오면 **먼저 이걸 묻는다.**

> **같은 셀(주·시장·품종·등급) 안에서 그 변수가 갈리는가?**

| | 셀 안에서 갈리는가 | 유효 단위 | 검정 가능? |
|---|---|---|---|
| 포장 규격·등급·출하시점·출하처 | **갈린다** | 셀 수천 개 | 잘 잡힌다 |
| 일교차·고도·토양 — 장소의 고정 성질 | **안 갈린다** | **시군 14개** | 안 잡힌다 |

안 갈리면 설계를 아무리 바꿔도 독립 단위가 14개다. 1-F가 통한 것은 포장이
**같은 시군 안에서도** 갈렸기 때문이다(셀·시군 조합의 14.1%).
일교차는 그 주 그 시군에 값이 하나뿐이라 안 갈린다.

**장소의 고정 성질을 검정할 수 있는 유일한 경로는 같은 시군의 시간 변동**이고,
그게 설계 3·4다(4,321주/5,028주, r=+0.024/−0.047, 최소 감지 ±1.3~2.1%).
**일교차 기각의 근거는 여기에 있고 지금도 유효하다.**

### 함정 하나 — 관측치 단위 부트스트랩

설계 7에서 시군x연도 92점으로 늘리자 규모가 **유의하게 나왔다**
(+0.0289 [+0.0049, +0.0473]). 시군 블록 부트스트랩으로 바로잡으니 판정불가가
됐다([−0.0249, +0.0647]). **행이 늘어난 것이지 독립 표본이 늘어난 것이 아니다.**
연도·주를 늘려 유의를 얻으면 같은 시군을 여러 번 센 것이다.

### 곁가지 — 규모 효과는 비례가 아니라 문턱일 수 있다

읍면 61개(104주 이상, 물량 99.4%)에서 규모 4분위별 상대값:

```
1분위(작음) 0.964 · 2분위 1.065 · 3분위 1.080 · 4분위(큼) 1.046
```

**클수록 비싼 것이 아니라 너무 작으면 싸다.** 2~4분위는 평평하다. 유의하진
않지만, 직선을 그으면 이 계단이 눌려 상관계수가 작아 보인다는 점은 기억할 것.

스크립트: `analyze_dtr_weighted.py` · `analyze_scale_dtr.py` · `analyze_dtr_matched.py`

---

## 1-F의 23%는 최근 1년 값이다 (2026-08-20 보완)

"2kg이 4kg보다 kg당 23% 더 받는다"를 전 기간으로 다시 재면 **11%**다.

```
최근 53주   셀 1,269   중앙값 1.260   평균 1.519
전 기간     셀 7,318   중앙값 1.107   평균 1.338
```

1-F 머리에 "최근 1년 기준"이 적혀 있어 틀린 기록은 아니지만, 23%라는 숫자만
떼어 쓰면 과거까지 일반화하게 된다. **소포장 프리미엄이 최근 두 배로 커졌다**는
것 자체가 볼 만한 건이다 — 추세인지 일시적인지는 아직 안 봤다.

---

## 검수 상태 — 앞 기록이 틀렸다

앞 판에 `audit_lettuce_daily.py` **전 항목 통과**라고 적혀 있는데 **사실이 아니다.**
커밋된 `outputs/audit_lettuce_daily.log`에도 실패 2건이 그대로 남아 있다.

```
[실패] 불일치 키 156개 (구에만 0, 신에만 156)
[실패] (일자,시장) 건수 불일치 101개
```

전부 **구(3일 표본)에 없고 신(전수)에만 있는** 방향이다. 즉 전수 자료가 더 많이
담고 있다는 뜻이지 전수가 뭘 잃었다는 뜻이 아니다. 걸린 것은 익산 망성면·남원
금지면·완주 봉동읍처럼 구 스크래퍼가 떨어뜨리던 라벨들이다.

**고쳐야 할 것은 자료가 아니라 감사 쪽 기준**으로 보이지만 확인은 안 했다.
다음 세션에서 판단할 것. `audit_hardcoded_lists.py`는 전 항목 확인 통과다.

---

## 바로 다음에 할 일 (우선순위)


1. **유통 문서 6절을 공개 사이트에 반영** — 농가수취율 20% → 25~27%로 고쳐 뒀다.
   아직 로컬에만 있다. 커밋·push가 필요하다(아래 「git 함정」 3번: 사람 터미널에서).
2. **`audit_lettuce_daily.py` 실패 2건 판정** — 위 「검수 상태」 참조.
   감사 기준을 고칠 일인지, 자료를 볼 일인지 결정한다.
3. **지역추천 재구성** — 토양적성도는 전북 상추 면적의 6.5%만 설명 (HANDOFF 9.5).
   `region_recommend.py`는 10작물 정리 때 같이 지웠다. 새로 세운다.
4. **소포장 프리미엄이 왜 두 배가 됐나** — 최근 53주 26% vs 전 기간 11%.
   추세인지 일시적 벌어짐인지, 품종·시장별로 다른지 (위 「1-F의 23%」 참조).
5. 표본 축적 후 기상 변수 재검정 (HANDOFF 10.4)

---

## 사용자가 정한 방침 (바꾸지 말 것)

- **상추 단일 작목.** `all_crops`는 신경 쓰지 않는다.
- **도매시장(katSale)만.** 산지공판장은 도매시장으로 재유입되므로 이중계상 (HANDOFF 1-B).
- **라벨을 지어내지 않는다.** 산지가 시군까지만 적혀 있으면 `익산시` 그대로 쓴다.
  `(미상)`·`읍면 미기재` 같은 말로 바꾸지 않는다.
- **단위를 반드시 명시.** 모든 가격은 원/kg, 상자단가는 `원/4kg 상자`로 병기.

---

## 반드시 지킬 방법론 (내가 두 번 어겨서 사고 냄)

### 목록은 **추측하지 말고 열거**한다

두 번 같은 실수를 했다.

| | 잘못한 것 | 결과 | 올바른 방법 |
|---|---|---|---|
| 시장 | 코드 접두를 추측해 훑고 "전수 탐색"이라 보고 | 안산·구미 누락 (30개로 착각) | `katRealTime2/trades2`를 **시장코드 없이 날짜만으로** 조회 |
| 품종 | 눈에 보이는 품종을 손으로 적음 | 5종 2.17% 누락 | **배제 목록 방식** — 뺄 것만 적고 나머지는 전부 포함 |

`audit_hardcoded_lists.py`를 돌리면 코드에 박힌 목록과 자료를 대조한다.
**새 목록을 추가하면 이 감사에도 추가할 것.**

열거가 불가능해 사람 판단이 들어간 3곳은 감사 출력에 명시돼 있다
(경쟁산지 지점 선택, 시군-지점 대체, 재배형태 추론).

### 성능 주장은 반드시 대조군과 함께

- 계절평균(또는 주차평균) 없이 MAPE만 제시하면 계절성 효과를 모델 성능으로 오인한다.
- 블록 부트스트랩 95% 구간이 0을 포함하면 **"판정불가"이지 "개선"이 아니다.**
- CV에서 고른 것을 홀드아웃 성적으로 다시 고르면 안 된다(rev2가 빠진 함정).

### 검정 전에 묻는다 — **같은 셀 안에서 갈리는 변수인가**

일교차를 여덟 설계로 검정하고 나서야 정리된 것이다(HANDOFF 1-E 설계 8).

| | 셀(주·시장·품종·등급) 안에서 갈리는가 | 유효 단위 | 검정 가능? |
|---|---|---|---|
| 포장 규격·등급·출하시점·출하처 | **갈린다** | 셀 수천 개 | 잘 잡힌다 |
| 일교차·고도·토양 — 장소의 고정 성질 | **안 갈린다** | **시군 14개** | 안 잡힌다 |

**안 갈리면 설계를 아무리 바꿔도 독립 단위가 14개다.** 1-F가 통한 것은 포장이
같은 시군 안에서도 갈렸기 때문이고(셀·시군 조합의 14.1%), 일교차가 계속 막힌
것은 그 주 그 시군에 값이 하나뿐이기 때문이다. 대응짝으로 옮겨도 안 된다.

장소의 고정 성질을 볼 수 있는 유일한 경로는 **같은 시군의 시간 변동**이다.

### 표본을 늘렸다고 착각하지 말 것 — 클러스터를 본다

시군x연도로 늘리면 92점이 되지만 독립 단위는 여전히 시군 13~14개다.
관측치 단위 부트스트랩으로 재면 규모가 **유의**하게 나오고(+0.0289
[+0.0049, +0.0473]), 시군 블록으로 바로잡으면 **판정불가**가 된다
([−0.0249, +0.0647]). 같은 시군을 여러 번 센 것일 뿐이다.

**물량가중도 표본을 늘리지 않는다.** 시군 14개를 물량으로 가중하면 Kish 유효표본이
**3.67**로 내려앉는다(남원 38% + 익산 32%). 계수가 커 보여도 N_eff를 같이 본다.

### 유의가 하나만 나오면 그 하나를 의심한다

설계 6에서 네 조합 중 하나만 구간이 0을 비껴갔다(r=−0.056, 상단 −0.003).
부호는 가설과 반대였고, **가중을 세게 할수록 계수가 커졌다**(동일 −0.002 →
q −0.056). 일반적 효과라면 가중치 변환에 안정해야 한다. 발견으로 세우지 않았다.

### 결과가 없으면 **양성 대조**로 장치를 확인한다

설계 8에서 일교차가 판정불가로 나왔는데, 같은 장치로 **이미 아는 효과**
(1-F 포장 +11%)를 되짚었더니 그것도 판정불가였다. 즉 "효과가 없다"가 아니라
**"이 설계가 못 잡는다"**였다. 감지 한계를 함께 적지 않으면 둘을 구별할 수 없다.

---

## 자주 쓰는 명령

```bash
cd "C:/Users/지리산 살래농장/Documents/project 1/crop-price-forecast/src"
P=../.venv/Scripts/python.exe

$P scrape_lettuce_daily.py --start 2018-01   # 수집(이어받기 자동)
$P audit_lettuce_daily.py                    # 수집 검수
$P audit_hardcoded_lists.py                  # 목록 감사
$P scrape_lettuce_daily.py --reaggregate     # 집계만 재생성

$P lettuce_cv.py compare                     # 월간 피처집합 비교
$P lettuce_weekly.py compare                 # 주간
$P test_long_horizon.py                      # 지평별 성적
$P forecast_2027.py                          # 2027 전망 생성
$P export_lettuce_webapp.py                  # 웹앱 JSON 생성 (~10분)

$P analyze_autumn_surge.py                   # 여름·가을 급등기 (기술통계+검정+export)
$P analyze_region_gap.py                     # 지역 가격차 원인 (포장 규격)
$P analyze_dtr_premium.py / _within / _grade / _threshold   # 일교차 설계 1~5 (기각)
$P analyze_dtr_weighted.py                   # 설계 6 — 물량가중 재검정
$P analyze_scale_dtr.py                      # 설계 7 — 규모와 함께 (사분면·읍면 61개)
$P analyze_dtr_matched.py                    # 설계 8 — 대응짝 + 양성 대조
$P analyze_namwon_shift.py                   # 남원 산간 전환의 정체 (라벨 이동)
$P fetch_at_margin.py                        # aT 품목별 유통비용 (상추 출하비용률)
$P fetch_retail_kamis.py                     # KAMIS 소매·도매 (최근 1년만 제공)
$P analyze_margin_chain.py                   # 산지·중도매·소매 3단계
$P export_destinations.py                    # 출하처 도매시장·품종 (기간별)
$P build_geo.py                              # 시군·읍면 경계 (원본은 data/raw)
```

웹앱이 읽는 파일은 5개다. `export_lettuce_webapp.py`만 10분 걸리고 나머지는 1~2분.
```
lettuce_app.json 1.2MB · lettuce_destinations.json 333KB · lettuce_forecast_2027.json 194KB
jeonbuk_eup_geo.json 172KB(드릴다운 때만) · lettuce_surge.json 18KB
```

예약 작업 `lettuce_daily_resume`가 매일 00:10에 수집을 이어받는다(완주 상태면 즉시 종료).

---

## API 키·쿼터

`.env`에 8개 (gitignore됨). 쿼터 실측치:

| API | 한도 | 비고 |
|---|---|---|
| katSale (data.go.kr) | **약 1,900회/일** | HANDOFF에 적혀 있던 "~1만"은 틀림 |
| ASOS (data.go.kr) | katSale과 **별개** | 같은 날 병행 수집 가능 |
| KMA API 허브 | 별도 키 `KMA_APIHUB_KEY` | API마다 활용신청 필요 |
| at.agromarket | `AT_AGROMARKET_KEY` | **남은 API가 산지공판장뿐이라 현재 용도 없음** |

katSale 전 구간 재수집은 약 2,100요청 / 70분.

---

## git 함정 (2026-08-12에 걸린 것)

**1. 이 폴더의 `.git`이 한 번 사라진 적 있다.** zip으로 옮기면서 빠졌다.
저장소는 멀쩡히 살아 있었는데 로컬만 연결이 끊겨 "git 설정을 안 했나" 싶은 상황이 됐다.
복구는 `git clone` 받아서 그 `.git`만 작업 폴더로 옮기면 된다. 파일은 손대지 않는다.

**2. 자격증명이 다른 계정으로 캐시돼 있다.** 저장소 주인은 `erato0310`인데
Windows 자격증명 관리자에는 `pilos050804`가 저장돼 있어서 push가 403으로 막혔다.
그래서 원격 주소에 계정을 박아 뒀다 — `https://erato0310@github.com/...`.

캐시를 지우고 다시 로그인하려면:
```bash
printf 'protocol=https\nhost=github.com\n\n' | git credential reject
```

**3. push는 반드시 사람이 쓰는 터미널에서 할 것.** 에이전트 셸에서는
자격증명 로그인 창을 띄우지 못해 그냥 멈춘다(5분 대기 후 타임아웃).
`/dev/tty`가 없어서 device 코드조차 표시하지 못한다.

**4. 로그인 창이 아예 안 뜨는 문제가 있었다.** GCM이 GUI 프롬프트를 못 띄웠다.
그래서 창 대신 **터미널에 코드가 찍히는 device 방식**으로 바꿔 뒀고, 그걸로 통과했다.
설정은 `.git/config`에 있으니 clone을 다시 받으면 또 해줘야 한다:
```bash
git config credential.gitHubAuthModes device
git config credential.guiPrompt false
```
push하면 터미널에 `github.com/login/device` 주소와 8자리 코드가 뜬다.
브라우저에서 그 코드를 넣으면 push가 이어진다.

**5. `core.autocrlf`가 시스템 전역에서 true다.** 이 저장소는 `false`로 덮어 놨다.
안 그러면 줄바꿈만 바뀐 파일이 수십 개씩 변경된 것처럼 잡힌다.
`git status`가 `M`인데 `git diff --numstat`에 안 나오면 stat 캐시 노이즈다.

---

## 사이트 구조 (2026-08-12 개편)

```
/                    → webapp/lettuce.html 로 이동
/webapp/             → lettuce.html 로 이동 (예전 10작물 앱 자리, 밖에 적어둔 주소라 살려 둠)
/webapp/lettuce.html 상추 앱 본체
```

10작물 앱(`app.js`·`style.css`·`dist/`·`app_data.json`)은 **사이트에서 내렸다.**
qty_kg 4배 부풀림 위에 계산된 숫자라 공개해 둘 수 없었다.
파일은 커밋 `2c1c9a1`에 남아 있으니 버그를 고치면 되살릴 수 있다.

---

## 마지막 검증 상태 (2026-08-12)

```
audit_lettuce_daily.py      전 항목 통과
audit_hardcoded_lists.py    전 항목 통과
월간 CV 23.28% / 홀드아웃 11.17%     (32개 시장·17품종 기준, 30개 시장과 동일)
주간 광 -0.52%p [-0.95, -0.10]      개선 판정 유지
웹앱 지도14·시군13·읍면63·차트·2027전망  전부 렌더 확인
공개 사이트 https://erato0310.github.io/crop-price-forecast/  (2026-08-14, 136c6f4)
  → 3단계 확대·설명 창 9개·품종 3분할·출하처 막대 전부 실서버 확인
품종 파일 대조         3품종 x 14시군 주수·물량이 원자료와 불일치 0건
회귀 검사              전체 화면이 개편 전과 동일(진안 6,008 / 전주 3,293 / 21,821t)
키 노출 검사           추적 파일 210개 x 키 7개 — 없음
```
