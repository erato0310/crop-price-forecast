...
미래예측 신뢰구간) → 2026-08-07 밤(새 환경 이전 + 데이터 대수술) →
**2026-08-11(kg 단위 통일 완료 — 바로 아래 절 참고)**. 이 프로젝트를
처음 보는 세션은 이 파일 → `README.md` → `docs/RESOLVE_GUIDE.md` → `src/models.py`
docstring 순으로 읽으면 지금까지의 경위·근거를 대부분 따라잡을 수 있다.

## ✅ kg 단위 통일 완료 (2026-08-11)

**핵심 결과: 유효 조합 93 → 88개, 중앙 CV MAPE 29.13% → 26.22%, MAPE≥100%인 조합 2개 → 1개,
모델이 베이스라인을 이긴 비율 67% → 77%.** 가설대로 대파·수박이 가장 크게 좋아졌다.

작물별 중앙 MAPE 변화(공통 88개 조합):

| 작물 | 상자단가 | 원/kg | 변화 |
|---|---|---|---|
| 대파 | 34.04% | 25.26% | **-8.77pp** |
| 멜론 | 28.77% | 23.18% | -5.59pp |
| 포도 | 33.24% | 28.25% | -5.00pp |
| 수박 | 32.10% | 28.16% | -3.94pp |
| 토마토·오이 | 26.0~26.3% | 25.5~25.7% | -0.6pp |
| 상추·방울토마토·고구마 | — | — | ±0.4pp(무변화) |
| 복숭아 | 30.79% | 32.24% | +1.45pp |

**예측 제외 확정이었던 2개가 되살아났다** — 수박×부안 188% → **71.2%**,
대파×김제 122% → **42.5%**. `EXCLUDE_MAPE=100` 기준을 자동으로 통과해 웹앱에 다시 들어온다.
다만 수박×부안은 베이스라인(42.7%)에 여전히 크게 지므로 신뢰도가 낮다는 점은 그대로다.
남은 MAPE≥100%는 방울토마토×진안(116.6%, 베이스라인도 89.6%) 하나뿐.

**대가**: 5개 조합이 관측 40개월 미만이 되어 빠졌다(오이×무주, 상추×임실, 멜론×남원,
복숭아×정읍, **복숭아×남원**). 특히 복숭아×남원은 MAPE 14.7%로 최상위였는데 아깝게 탈락했다 —
표본이 더 쌓이면 자동으로 돌아온다.

### 이번 세션에서 발견해 고친 것 (전부 실측 근거 있음)

1. **`unit_qty` 이상치가 물량가중평균을 납치** — `unit_qty`는 거래 단위 1개의 kg 중량이어야
   하는데 극소수 레코드가 3001·805·813 같은 값을 담고 있다. 두 방향으로 동시에 틀려서 치명적:
   `price_kg = price/unit_qty`는 수백 분의 1로 찌그러지고 `qty_kg = qty*unit_qty`는 수백 배로
   부풀어, **그 한 행이 그 달 전체 평균을 지배한다.** 실측: 오이×임실 2022-11이 정상 36건
   (1,000~2,000원/kg) + 불량 3건 때문에 월평균 **4.85원/kg**으로 붕괴 → CV MAPE 21.5% → 158%.
   포도×전주 2021-01도 52원/kg. TOP10 작물에서 `unit_qty>50`은 272,434건 중 **18건**뿐인데
   그 18건이 kg 물량 총합의 **35.9%**를 차지했다. → `MAX_UNIT_QTY_KG=50` 상한 도입
   (`scrape_jeonbuk_all_crops._kg_fields`). 하한은 두지 않았다 — 0.2~0.5kg 소포장은 실제로
   존재하고 그 경우 높은 원/kg이 맞다.
2. **거래 1건짜리 월평균이 lag 피처를 오염** — 진안 토마토 2020-02·03이 각각 1건이라
   11,333·16,000원/kg(두터운 달 중앙값 2,374원/kg의 5~7배)로 잡혀 CV가 26% → 135%로 터졌다.
   → `build_dataset.MIN_OBS_PER_MONTH=2`. 값은 CV로 정했고(`tune_min_obs.py`,
   로그 `outputs/min_obs_threshold_cv.log`) **임계값이 오르면 어려운 조합이 유효 목록에서
   빠져 중앙 MAPE가 저절로 좋아지는 함정**이 있어 공통 70개 조합으로 재비교했다:
   ...
   3~5는 정확도가 포화되는데 조합을 12~18개 더 잃는다. 정확도를 더 원하면 3으로 올리면
   되지만 웹앱 노출이 88 → 76으로 준다.
3. **재개(resume) 로직이 다음번엔 죽는 상태였다** — 체크포인트를 다시 읽으면 date가
   Timestamp가 되는데 새로 받는 행은 `datetime.date`라, 섞어 저장하면 한 CSV에
   `"2018-01-05 00:00:00"`과 `"2018-01-05"`이 공존한다. 그러면 다음 재개 때
   `read_csv(parse_dates=)`가 파싱을 포기하고 str로 돌려줘 `.dt`에서 AttributeError로 죽는다
   (pandas 3.0.5 실측). 읽자마자 타입을 맞추도록 고쳤다.
4. **네트워크 실패 구멍 메우기** — 기록만 하고 방치되던 실패 요청(9건, 2021-09-15에 6개
   시장 / 2024-06-05에 3건)을 `backfill_failed_days.py`로 재수집. **해당 (시장,날짜)의
   기존 행을 지우고 갈아끼우는** 방식이라 페이지 단위 부분 실패에서도 중복이 안 생긴다.
   9/9 복구, 원시 610,139 → 611,108행.
5. **중복 검사 결과: 없음.** 78개월 시점에서 검사했을 때 표본일(3일)보다 많은 날짜가 담긴
   달이 0개였고, 완전 동일 행 0.67%는 배수 분포에 홀수가 섞여 있어(3배 74건, 5배 6건)
   실제 거래다. 완주 시 자동으로 같은 검사를 하도록 `_report_integrity()`를 넣었다.

### 환경 검증 (중요)

새 `.venv`(Python 3.12.10, **pandas 3.0.5**)에서 **데이터를 바꾸기 전에** 기존 패널로
`crop_county_cv.py`를 다시 돌려 2026-08-10 결과와 **93개 조합 전부 소수점 13자리까지 일치**
함을 확인했다(`outputs/crop_county_cv_envcheck.log`). pandas 메이저 버전이 올라갔는데도
결과가 동일하므로, 위 CV 변화는 전부 데이터 변경 효과다. requirements.txt를 핀할 필요 없음.

### 비교용 백업 파일 (헷갈리지 말 것)

- `crop_county_cv_summary_before_rescrape.csv` — 재스크래핑 **전** + 상자단가 (84개 조합)
- `crop_county_cv_summary_before_kg.csv` — 재스크래핑 **후** + 상자단가 (93개 조합)
  ← **kg 효과만 떼어 보려면 이것과 비교**
- `crop_county_cv_summary.csv` — 현재(재스크래핑 후 + 원/kg + 표본필터, 88개 조합)
- `data/raw/backup_top10crops_before_kg.csv` / `backup_top10crops_before_unitfix.csv`

### 남은 일

1. **웹앱 재배포** — `webapp/`이 kg 기준으로 갱신됐다(87개 조합, `app_data.json` 173KB).
   GitHub Pages에 push하면 끝. 저장소 https://github.com/erato0310/crop-price-forecast
2. 전북 전체 축(`price_avg_jeonbuk`, `models.py`/`backtest.py`/`forecast_future.py`)은
   **아직 상자단가 그대로다.** 시장 위치 기준 스크래퍼(`scrape_jeonbuk_market.py`)에
   kg 필드가 없어서 손대지 않았다. 웹앱은 작물×시군 축만 쓰므로 당장 문제는 없지만,
   전북 전체 수치를 kg으로 같이 보이려면 그 스크래퍼부터 고쳐야 한다.
3. `docs/진행보고서_전북농산물가격예측AI.docx`의 수치가 상자단가 시절 것이다 — 갱신 필요.

### 실행 순서 (kg 기준, 현재)

```bash
cd crop-price-forecast/src
..\.venv\Scripts\python.exe scrape_jeonbuk_all_crops.py    # 신규 월만 이어받음, ~5시간
..\.venv\Scripts\python.exe backfill_failed_days.py        # 실패한 (시장,날짜) 구멍 메우기
..\.venv\Scripts\python.exe build_dataset.py               # ~10분
..\.venv\Scripts\python.exe crop_county_cv.py              # 수 분
..\.venv\Scripts\python.exe crop_county_predict.py
..\.venv\Scripts\python.exe crop_county_forecast.py
..\.venv\Scripts\python.exe export_webapp_data.py
..\.venv\Scripts\python.exe build_webapp.py
```

kg 환산 규칙(`_kg_fields`)만 바꿨다면 재스크래핑 없이
`scrape_jeonbuk_all_crops.py --reaggregate`로 원시 파일에서 다시 계산하면 된다.

### kg 단위 통일 — 왜 하는가 (근거)

사용자가 "익산 공판장에서 상추 10만원 넘게 나왔는데 데이터가 다르다"고 지적한 데서 출발했다.
조사 결과 세 가지가 드러났다.

1. **10만원 기록은 실재한다** — 익산산 상추 138,000원(2024-10-05, 군산원예농협, 적상추 4kg 상자).
   2025년 최고는 46,500원이라 사용자가 기억한 연도만 달랐다.
2. **단위가 뒤섞여 있었다.** katSale `avgprc`는 *거래 단위(상자)당* 단가인데 상자 무게가
   작물·거래마다 다르다. 실측(표본 8,984건):
   - 수박 unit_qty 3~12kg → 상자단가 변동 35배 vs **kg 환산 11배**
   - 대파 1~15kg → 18배 vs **6배**
   - 상추 2~5kg → 74배 vs **37배**
   HANDOFF에 "수박은 통/kg 혼입으로 예측 제외 권장"이라 적어둔 그 오염의 정체가 이것이다.
   **kg 환산으로 해결된다.** `unit_nm`은 표본에서 100% 'kg'이고 `unit_qty`가 상자 무게다.
   그래서 스크래퍼에 `price_kg = avgprc / unit_qty`, `qty_kg = qty * unit_qty`를 추가했다.
3. **공판장(gpj)은 이미 원/kg이었다.** 그래서 웹앱에서 두 소스를 같은 "원"으로 나란히
   보여준 것이 오독을 유발했다(군산 1,741원/kg이 "제일 싼 곳"으로 보임). kg으로 통일하면
   이 문제 자체가 사라진다.

### 공판장 API 중복 버그 (해결됨, 재발 감시용 기록)

농식품부 산지공판장 API가 **특정 날짜에 모든 행을 정확히 2배(2026-07-15은 4배)로 돌려준다.**
우리 스크래퍼 버그가 아니다 — 같은 날짜를 새로 호출해도 재현된다.

- **정상 중복과 구분하는 법**: 같은 날 동일 조건 거래가 여러 건인 건 정상이다
  (2021~2024년 중복률 17~24%, 배수 분포에 홀수가 대부분). 반면 버그 난 날짜는
  **모든 그룹의 배수가 짝수이고 홀수가 하나도 없다**. 그래서 "홀수 배수 0건이면 부풀림"으로
  판정하고 홀수가 나타날 때까지 반복해 절반으로 줄인다.
  **무조건 drop_duplicates 하면 진짜 거래가 지워진다 — 절대 금지.**
- 영향받은 날짜 10개(2025-05-15 ~ 2026-07-15), 11,999행 제거.
- 수정 전 영향: 가격은 395개 조합 중 94개에서 변동(평균 0.69%, 최대 12.9%),
  거래물량은 135개 조합에서 부풀려짐(평균 1.19배, 최대 5.6배 — qty_log가 모델 피처라 중요).
- 원본 백업: `data/raw/gongpanjang_jeonbuk_raw_before_dupfix.csv`

---

## ⏸ 중단 상태에서 재개하기 (2026-08-10 낮 — 이전 단계, 완료됨)
