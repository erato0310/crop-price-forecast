...
- 모든 가격이 **원/kg**으로 통일돼 있어 도매시장·산지공판장 자료를 같은 색 척도에 올립니다
  (예전에는 도매시장이 상자당 단가라 공판장 시군을 척도에서 빼고 빗금으로 그렸습니다).
  자료원이 산지공판장인 항목은 표에 `공판장` 표시가 붙습니다.
...
- **가격 단위**: 모두 **원/kg**입니다. 도매시장 정산정보의 단가는 상자 등 거래 단위당
  금액이라, 거래별 포장 중량(`unit_qty`)으로 나눠 kg 단가로 환산한 뒤 kg 물량으로
  가중평균했습니다. 등급·품종 차이는 평균 안에 섞여 있습니다.
- **군산 등 일부 시군**은 도매시장 표본이 부족해 산지공판장 정산가를 대신 사용합니다 —
  단위는 같은 원/kg이지만 정산 방식이 달라(경매 vs 계통출하 정산) 수준 차이가 날 수 있습니다.
